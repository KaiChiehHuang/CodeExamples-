...
struct PeakDetector {
    ...
    float	b0_r, a1_r, b0_a, a1_a, levelEstimate, pRoot;
    ...
    void processPeak (float input, float& output) {
    ...
    void processRMS (float input, float& output) {
        levelEstimate += b0_r * ( pow(fabs( input ),2) - levelEstimate );
        output = sqrt(levelEstimate);
    }
    void processRMP (float input, float& output) {
        levelEstimate += b0_r * ( pow(fabs( input ),pRoot) - levelEstimate );
        output = pow(levelEstimate,1./pRoot);
    }
};
...
public:
	...
	void  bilinearTranform(float acoefs[], float dcoeffs[], float center);
...
protected:
    ...
    PeakDetector levelDetector; // level detector 
